export function mountHobbyScroll(section) {
  ...
  const update = () => {
    ...
    viewport.scrollLeft = Math.max(
      0,
      Math.min(travel, top - section.getBoundingClientRect().top),
    );
  };
  const schedule = () => {
  ...
  const measure = () => {
    ...
    section.style.height = preference.matches
      ? ''
      : `${viewport.clientHeight + travel}px`;
  ...
  viewport.addEventListener(
    ...
    (event) => {
      ...
      if (preference.matches)
        viewport.scrollTo({ left: destination, behavior: 'auto' });
      else {
        const top = parseFloat(getComputedStyle(viewport).top) || 0;
        window.scrollTo({
          top:
            scrollY + section.getBoundingClientRect().top - top + destination,
          behavior: 'instant',
        });
      }
    },
