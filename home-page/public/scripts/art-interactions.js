import { createResponsiveImage, imageSizes } from './responsive-images.js';

const rows = {
  navigation: [
    {
      image: 'imgCursor1.png',
      label: 'Explore my work',
      text: 'Go check out my very cool projects!',
      href: '#work',
      color: 'blue',
    },
    {
...
export function mountArtRow(container, kind) {
  ...
  const position = () => {
    ...
    const navigationBottom =
      document.querySelector('.navigation')?.getBoundingClientRect().bottom ??
      0;
    ...
    bubble.style.setProperty(
      '--pointer-left',
      `${Math.max(22, Math.min(anchor.left + anchor.width / 2 - left, bounds.width - 30))}px`,
    );
  };
  ...
  for (const entry of rows[kind]) {
    ...
    if (kind === 'navigation' && !entry.href)
      trigger.setAttribute('aria-haspopup', 'menu');
    const image = createResponsiveImage(
      `/images/${entry.image}`,
      imageSizes.illustration,
      kind === 'navigation' ? 'eager' : 'lazy',
    );
    image.alt = '';
    image.className = 'art';
    image.width = 1000;
      ...
      () => {
        if (kind === 'quotes') {
          ...
          if (!entry.href) {
            ...
            const menu =
              contact &&
              document.getElementById(contact.getAttribute('aria-controls'));
  ...
  document.addEventListener(
    'pointerdown',
    (event) => {
      if (!container.contains(event.target) && !bubble.contains(event.target))
        hide();
    },
    options,
  );
